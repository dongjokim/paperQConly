...
\end{equation}
where the curly brackets denote the average over the transverse plane, i.e. $\{\cdots\}$ = $\int$ d$x$d$y$\, $e(x,y,\tau_0)$ $(\cdots)$, $r$ is the distance to the system's center of mass, $e(x,y,\tau_0)$ is the energy density at the initial time $\tau_0$, and $\Phi_{n}$ is the participant plane angle (see Refs.~\cite{Teaney:2010vd,Niemi:2015qia}).
...
In Sec.~\ref{sec:method} we summarize our findings from the previous work~\cite{ALICE:2016kpq} and present the analysis methods. The experimental setup and measurements are described in Sec.~\ref{sec:experiment}. The sources of systematic uncertainties are explained in Sec.~\ref{sec:uncertainties}. The results of the measurements are presented in Sec.~\ref{sec:results}. In Sec.~\ref{sec:theory} we present comparisons to model calculations.
...
The data samples of $\PbPb$ collisions at the centre-of-mass energy $\snn=2.76$~TeV analyzed in this article were recorded by ALICE during the 2010 heavy-ion run of the LHC.
...
\begin{figure*}
	\begin{center}
        	\resizebox{0.92\columnwidth}{!}{\includegraphics{figs/Fig2A_vn_xpt.eps}}
...
For the data comparisons three different configurations of the AMPT model has been used: the default one and string melting with and without hadronic rescattering. The input parameters used in all configurations are: $\alpha_s = 0.33$, a partonic cross-section of 1.5~mb, while the Lund string fragmentation parameters were set to $\alpha = 0.5$ and $b = 0.9$~GeV$^{-2}$.
In the default configuration, partons are recombined with their parent strings when they stop interacting. The resulting strings are later converted into hadrons using the Lund string fragmentation model~\cite{Andersson:1986gw,NilssonAlmqvist:1986rx}. In the string melting configuration, the initial strings are melted into partons whose interactions are described by the ZPC parton cascade model~\cite{Zhang:1997ej}. These partons are then combined into the final state hadrons via a quark coalescence model. 
...
\begin{figure*}[!]
            \begin{center}
                       \resizebox{\multfigsize\textwidth}{!}{\includegraphics{figs/Fig3_ModelComparison_EKRT.eps}}
              \end{center}
...
\begin{figure*}[!]
	\begin{center}
        	\resizebox{\multfigsize\columnwidth}{!}{\includegraphics{figs/Fig4_ModelComparison_VISH.eps}}
...
The SC(4,3) results are clearly favored by smaller $\eta/s$ values but NSC(4,3) cannot be described by these models quantitively.
 ...
 \begin{figure*}[!]
	\begin{center}
        	\resizebox{\multfigsize\columnwidth}{!}{\includegraphics{figs/Fig5_ModelComparison_AMPT.eps}}
...
The agreement of various model calculations with the data is quantified using the log-likelihood or least squares-${\chi}^2$ method~\cite{PDBook} by calculating the ${\chi}^2$:
...
The total number of data samples $N$ in Eq.~\ref{Eq:chisq} is 4, which corresponds to the number of bins in the centrality range 10--50\% used in ${\chi}^2$ calculations.
The ${\chi}^2$ for model calculations which are best in describing the SC observables for each of the three different types of models is shown in Fig.~\ref{fig:Figure_A5}.

\begin{figure*}[h]
            \begin{center}
                       \resizebox{0.99\textwidth}{!}{\includegraphics{figs/chisq_bestfits_3pad.eps}}
        \caption{The ${\chi}^2$ values calculated by Eq.~\ref{Eq:chisq} are shown for SC($m$,$n$) (a), NSC($m$,$n$) (b) and individual harmonics $v_n$ (c). Results are for model calculations which are best in describing the SC observables for each of the three different types of models.}
        \label{fig:Figure_A5}
              \end{center}
\end{figure*}
% The ${\chi}^2$ results for SC($m$,$n$) (NSC($m$,$n$)) are shown as closed (open) markers.
The results for SC($m$,$n$) and NSC($m$,$n$) are presented in Fig.~\ref{fig:Figure_A5}a and \ref{fig:Figure_A5}b, respectively. The ${\chi}^2$ values for the individual flow harmonics $v_n$ ($n$ = 2, 3 and 4) are shown in Fig.~\ref{fig:Figure_A5}c. We found that in case of the calculations from VISH2+1 with AMPT initial conditions ($\eta/s$ = 0.08) and default AMPT models, the ${\chi}^2$ values for SC($m$,$n$) are larger than those for NSC($m$,$n$). This reflects the fact that the individual flow harmonics $v_n$ are not well described by those models compared to event-by-event EKRT+viscous hydrodynamics. This is quantified in Fig.~\ref{fig:Figure_A5}c, where the ${\chi}^2$ values for $v_n$ are much larger both for VISH2+1 and default AMPT calculations than event-by-event EKRT+viscous hydrodynamics.
The default AMPT model gives best ${\chi}^2$ values for NSC($m$,$n$), especially for NSC(3,2). However the ${\chi}^2$ values of this model are largest for $v_n$ among the models especially for $v_2$.

The ${\chi}^2$ values for $v_2$ and $v_3$ are significantly smaller than those for SC(3,2) and NSC(3,2) for all the hydrodynamic calculations. The ${\chi}^2$ values for SC(4,2) and NSC(4,2) from event-by-event EKRT+viscous hydrodynamics are comparable to that for $v_2$ but larger than for $v_4$. This illustrates that the SC observables together with the individual flow harmonics provide better constraints for the model parameters than each of them individually.
%This finding supports the idea that the SC observables can provide better constraints in the model calculations than the individual flow harmonics $v_n$. 

%As discussed in \cite{ALICE:2016kpq}, the other $\eta/s(T)$ parameterizations in event-by-event EKRT+viscous hydrodynamics give worse descriptions for SC(4,2) and NSC(4,2) compared to ``param1''. 
...
Even with the best model calculations from theoretical models, the ${\chi}^2$ value varies a lot depending on the model parameters and/or different order SC observables, which implies that the different order harmonic correlations have different sensitivity to the initial conditions and the system properties.

\subsection{Transverse Momentum Dependence of Correlations between $v_2$ and $v_3$ and $v_2$ and $v_4$}
\label{sec:ptdepsc}
...
\begin{figure}[p]
             \begin{center}
              \resizebox{0.95\textwidth}{!}{\includegraphics{figs/Fig6_pt_NSC32_Models.eps}}
              \end{center}
...
\begin{figure}[p]
             \begin{center}
              \resizebox{0.95\textwidth}{!}{\includegraphics{figs/Fig7_pt_NSC42_Models.eps}}
              \end{center}
...
The default configuration of the AMPT is well reproducing NSC(3,2), while the other AMPT configurations predict a very strong $p_{\rm T}$ dependence above 1~GeV/$c$ and cannot describe the magnitudes of both NSC(3,2) and NSC(4,2) simultaneously.
