...
To study the $p_{\rm T}$ dependence of SC$(m,n)$, we show the results as a function of the low $p_{\rm T}$ cut-off ($p_{\rm T, min}$), instead of using independent $p_{\rm T}$ intervals, in order to avoid large statistical fluctuations in the results. Various minimum $p_{\rm T}$ cuts from 0.2 to 1.5~GeV/$c$ are applied.
...
The complete sets of the individual flow harmonics $v_n$ used to calculate the NSC observables are shown in Fig.~\ref{fig:Figure_2A}. The centrality dependence of $v_n$ ($n$ = 2, 3, 4 and 5) is shown in the left panels ((A), (B) and (C)) of  Fig.~\ref{fig:Figure_2A}. The $v_n$ values ($n < 5$) are equivalent to those in \cite{Adam:2016izf}. The fifth order flow harmonic $v_5$ is shown in panel (C). The $p_{\rm T, min}$ dependence of $v_n$ ($n$ = 2, 3 and 4) is shown in the right panels ((a), (b) and (c)) of Fig.~\ref{fig:Figure_2A} in all centrality ranges relevant to the measured NSC($m$,$n$) observables.
...
Three different models are compared with the experimental results presented in Sec.~\ref{sec:results}. First, we have compared our observables with event-by-event EKRT+viscous hydrodynamic predictions with the different parameterizations for the temperature dependence of the shear viscosity to entropy density ratio $\eta/s(T)$ are shown in Fig.~2 of Ref.~\cite{ALICE:2016kpq}. 
