...
The existing measurements provide an estimate of the average value of QGP's $\eta/s$, both at RHIC and LHC energies. What remains uncertain is how the $\eta/s$ of QGP depends on temperature ($T$)~\cite{Niemi:2015qia}. This study has been just initiated by the theorists in Ref.~\cite{Niemi:2015qia}, where the first (and only rather qualitative) possibilities where investigated (see Fig.~1 therein). The emerging picture is that the study of individual flow harmonics $v_n$ will unlikely reveal the details of $\eta/s(T)$ dependence.
