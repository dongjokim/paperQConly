...
The strong $p_{\rm T}$ dependence observed in SC$(m,n)$ is not seen in NSC$(m,n)$. The NSC$(m,n)$ results are consistent within errors for all minimum $p_{\rm T}$ cuts. 
...
The $p_{\rm T}$-dependence of NSC(3,2) is not clearly seen and it is consistent with no $p_{\rm T}$ dependence within the current statistical and systematic errors for the centrality range $<$30\%, while showing a moderate decreasing trend with increasing $p_{\rm T}$ for the $>$30\% centrality range.
...
While we discussed the comparison to these hydrodynamic model calculations with various temperature dependent $\eta/s$ parameterizations, only two calculations with the parameters which describe the lower order harmonic correlations better are compared to the results of higher order harmonic correlations. The other calculations show a quite clear deviation from the data. 
As it is seen in Fig.~1 from Ref.~\cite{Niemi:2015qia} that the temperature at the minimum of $\eta/s$ for "\textit{param1}" (at $T$ = 150~MeV) is smaller than the other parameters used. The model calculations with the parameters whose temperature at the minimum of $\eta/s$ is larger than "param1" can be ruled out.
As shown in Fig.~\ref{fig:Figure_5}, the correlations between $v_5$ and $v_2$ and between $v_5$ and $v_3$ are well described by those two model parameters within the errors.  The model calculations for SC(5,2) and SC(5,3) give better description than the correlations between $v_4$ and $v_2$. 
...
%\begin{figure*}[!]
%            \begin{center}
%                       \resizebox{0.32\textwidth}{!}{\includegraphics{figs/fig3_QConly_ModelComparison_SC32_EKRT.eps}}
%                       \resizebox{0.32\textwidth}{!}{\includegraphics{figs/fig3_QConly_ModelComparison_SC42_EKRT.eps}}
%                \end{center}
...
\begin{figure*}[!]
            \begin{center}
              ...
              \end{center}
        \caption{Measurements of  SC(5,2), SC(5,3) and SC(4,3) in $\PbPb$ collisions at $\snn=2.76$~TeV are compared to the event-by-event EKRT+viscous hydrodynamic calculations~\cite{Niemi:2015qia}. The lines are hydrodynamic predictions with $\eta/s(T)$ parameterizations~\cite{Niemi:2015qia}.}
        \label{fig:Figure_5}
\end{figure*}
...
\begin{enumerate}[(i)]
	...
	\item Among the VISH2+1 model calculations with small $\eta/s$ ($\eta/s=0.08$), the one with the AMPT initial conditions describes the data qualitatively but it cannot describe the data quantitively for most centrality ranges.
...
In order to see the trend more clearly, we show NSC($m$,$n$) results as a function of minimum $p_{\rm T}$ cut in Fig.~\ref{fig:Figure_8}.
...
This observed moderate $p_{\rm T}$ dependence in mid-central collisions both for NSC(3,2) and NSC(4,2) might be an indication of possible viscous corrections to the equilibrium distribution at hadronic freeze-out as predicted in ~\cite{Luzum:2010ad}.
