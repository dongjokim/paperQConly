...
The first results for the new multiparticle observables which quantify the relationship between event-by-event fluctuations of two different flow harmonics, the so-called \textit{Symmetric Cumulants}~(SC), were recently reported by the ALICE Collaboration~\cite{ALICE:2016kpq}.
...
The complete sets of the individual flow harmonics $v_n$ used to calculate the NSC observables are shown in Fig.~\ref{fig:Figure_2A}. The centrality dependence of $v_n$ (n=2, 3, 4 and 5) is shown on the left panels ((A), (B) and (C)) of  Fig.~\ref{fig:Figure_2A}. The results of $v_n$ (n $<$ 5) are equivalent to those in \cite{Adam:2016izf}. The fifth order flow harmonics $v_5$ is shown on the panel (C). The $p_{\rm T, min}$ dependence of $v_n$ (n=2, 3 and 4) is shown in the right panels ((a), (b) and (c)) of Fig.~\ref{fig:Figure_2A} in all centrality ranges relevant to the measured NSC($m$,$n$) observables.
