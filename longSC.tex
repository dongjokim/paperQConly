...
The first results for a new multiparticle observables which quantify the relationship between event-by-event fluctuations of two different flow harmonics, the so-called \textit{Symmetric Cumulants}~(SC), were recently reported by the ALICE Collaboration~\cite{ALICE:2016kpq}.
...
%Secondly, the HIJING model was used to estimate the strength of nonflow correlations (typically few-particle correlations insensitive to the collision geometry). 
...
While the centrality dependence of the SC observables is compared to various models, it is still worthwhile to do quantitative comparison of individual flow harmonics $v_n$ to the various models.
