...
\begin{figure*}[h]
            \begin{center}
                       \resizebox{0.99\textwidth}{!}{\includegraphics{figs/chisq_bestfits_3pad_wo_ampt.eps}}
        \caption{The ${\chi}^2$ values from the comparison of few selected calculations from two viscous hydrodynamic models are shown for SC($m$,$n$) (a),  NSC($m$,$n$) (b) and individual harmonics $v_n$ (c).}
        %The ${\chi}^2$ results for SC($m$,$n$) (NSC($m$,$n$)) are shown as closed (open) markers.}
        \label{fig:Figure_A5}
              \end{center}
\end{figure*}

%The results for SC($m$,$n$) and NSC($m$,$n$) are presented in Fig~\ref{fig:Figure_A5}a and Fig~\ref{fig:Figure_A5}b, respectively. The ${\chi}^2$ values for the individual flow harmonics $v_n$ ($n$ = 2, 3 and 4) are shown in Fig~\ref{fig:Figure_A5}c. We found that in case of the calculations from VISH2+1 with AMPT initial conditions ($\eta/s$ = 0.08) and default AMPT models, the ${\chi}^2$ values for SC($m$,$n$) is larger than those for NSC($m$,$n$). This can be explained by the fact that the individual flow harmonics $v_n$ is not well described by those models compared to the event-by-event EKRT+viscous hydrodynamics. This is quantified in Fig.~\ref{fig:Figure_A5}c, showing that the ${\chi}^2$ values for $v_n$ are much larger both for the VISH2+1 and default AMPT calculations than event-by-event EKRT+viscous hydrodynamics.
%The default AMPT model gives best ${\chi}^2$ values for NSC($m$,$n$), especially for NSC(3,2). However the ${\chi}^2$ values are worst for $v_n$ among the models.
