...
Finally, we perform a least-square test to the models for each SC observable via a log-likelihood or least squares-${\chi}^2$ method~\cite{PDBook} as following,
...
%The resulting ${\chi}^2$ for few selected calculations from two viscous hydrodynamic models which describe the SC observables best is shown in Fig.~\ref{fig:Figure_A5}.

\begin{figure*}[h]
            \begin{center}
                       \resizebox{0.99\textwidth}{!}{\includegraphics{figs/chisq_bestfits_3pad.eps}}
        \caption{The ${\chi}^2$ values from the comparison of few selected calculations from three theoretical models are shown for SC($m$,$n$) (a),  NSC($m$,$n$) (b) and individual harmonics $v_n$ (c). The ${\chi}^2$ results for SC($m$,$n$) (NSC($m$,$n$)) are shown as closed (open) markers.}
        \label{fig:Figure_A5}
              \end{center}
\end{figure*}

The results for SC($m$,$n$) and NSC($m$,$n$) are presented in Fig.~\ref{fig:Figure_A5}a and \ref{fig:Figure_A5}b, respectively. The ${\chi}^2$ values for the individual flow harmonics $v_n$ ($n$ = 2, 3 and 4) are shown in Fig.~\ref{fig:Figure_A5}c. We found that in case of the calculations from VISH2+1 with AMPT initial conditions ($\eta/s$ = 0.08) and default AMPT models, the ${\chi}^2$ values for SC($m$,$n$) are larger than those for NSC($m$,$n$). This can be explained by the fact that the individual flow harmonics $v_n$ are not well described by those models compared to event-by-event EKRT+viscous hydrodynamics. This is quantified in Fig.~\ref{fig:Figure_A5}c, showing that the ${\chi}^2$ values for $v_n$ are much larger both for VISH2+1 and default AMPT calculations than event-by-event EKRT+viscous hydrodynamics.
The default AMPT model gives best ${\chi}^2$ values for NSC($m$,$n$), especially for NSC(3,2). However the ${\chi}^2$ values of this model are worst for $v_n$ among the models and noticeably larger for $v_2$.

The ${\chi}^2$ values for $v_2$ and $v_3$ are significantly smaller than those for SC(3,2) and NSC(3,2) for all the hydrodynamic calculations. As for the calculations from event-by-event EKRT+viscous hydrodynamics, the ${\chi}^2$ values for SC(4,2) and NSC(4,2) are comparable to that for $v_2$ but they are larger than that for $v_4$. This finding supports the idea that the SC observables can provide better constraints in the model calculations than the individual flow harmonics $v_n$. 
