...
\begin{equation}
\chi^{2} = \sum_{i=0}^{nbins} \frac{{(y_i-f_i)}^{2}}{{\sigma_i}^{2}},
\label{Eq:chisq}
\end{equation}
where $y_i$ is a measurement in a centrality bin, and the systematic and statistical errors  are combined in quadrature $\sigma_i = \sqrt{\sigma_{i, stat}^2 + \sigma_{i, syst}^2}$.
The resulting ${\chi}^2$ for few selected calculations from three theoretical models which describe the SC obserables best is shown in Fig.~\ref{fig:Figure_A5}.
\begin{figure*}[h]
            \begin{center}
                       \resizebox{0.8\textwidth}{!}{\includegraphics{figs/chisq_bestfits.eps}}
        \caption{The resulting ${\chi}^2$ from the comparison of few selected calculations from three theoretical models are shown for each SC observable. Closed (open) markers show SC($m$,$n$) (NSC($m$,$n$).}
        \label{fig:Figure_A5}
              \end{center}
\end{figure*}

We found that the overall ${\chi}^2$ for SC($m$,$n$) is larger than one for NSC($m$,$n$) and ${\chi}^2$. 
Even with the best model calculations from three theoretical models, ${\chi}^2$ varies a lot depending on the models, which implies that the different order harmonic correlations have different sensitivities to the initial conditions and the system properties. 
