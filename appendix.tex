...
\begin{figure*}[h]
        ...
        \caption{SC and NSC obserables in $\PbPb$ collisions at $\snn=2.76$~TeV~\cite{Adam:2016izf} are compared to few selected model calculations from three theoretical models which describe the data best.}
        \label{fig:Figure_A5}
              \end{center}
\end{figure*}

The chi-square is calculated for each observable as following, 

\begin{equation}
\chi^{2} = \sum_{i=0}^{nbins} (\frac{y_i-f_i}{\sigma_i})^{2}  \\
\sigma_i = \sqrt{\sigma_{i, stat}^2 + \sigma_{i, syst}^2}
\label{Eq:chisq}
\end{equation}
